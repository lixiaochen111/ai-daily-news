     ...
     [黑名单]     [Tier 0]     [Tier 1]       [Tier 2]
      删除        编辑精选      高质量源        广域源
          ...
          X            │            │              ▼
                  ...
                  │                                     │
                  │ 分析输出:                           │
                  │  - design_relevance (0-10分)       │
                  │  - quality_score (0-10分)          │
                  │  - tags (标签数组)                  │
                  │  - reasoning (分析理由)             │
                  │                                     │
                  │ 过滤阈值（不同tier标准不同）:        │
                  │  - Tier 1: relevance >= 6/10      │
                  │    (更宽松，不漏好内容)             │
                  │  - Tier 2: relevance >= 7/10      │
                  │    (更严格，精选内容)               │
                  └──────────────┬──────────────────────┘
                                 │
                                 ▼
...
| 维度 | Tier 0 | Tier 1 | Tier 2 |
|------|--------|--------|--------|
| **内容特点** | 编辑精选 | 高质量但杂 | 内容广泛 |
| **输入量** | 40条/天 | 10条/天 | 100条/天 |
| **筛选策略** | 无需筛选 | 仅深度分析 | 三阶段筛选 |
| **关键词筛选** | ❌ 跳过 | ❌ 跳过 | ✅ 100→30条 |
| **GLM初筛** | ❌ 跳过 | ❌ 跳过 | ✅ 30→15条 |
| **深度分析** | ❌ 跳过 | ✅ 10条 | ✅ 15条 |
| **过滤阈值** | - | relevance≥6 | relevance≥7 |
| **输出量** | 40条 | 6条 | 10条 |
| **日成本** | ¥0 | ¥0.02 | ¥0.06 |
| **设计原则** | 信任编辑 | 精准不漏 | 成本优化 |
...
- **Tier 0**: 编辑已筛选，AI不要画蛇添足
- **Tier 1**: 内容少且精，一次深度分析到位
- **Tier 2**: 内容杂且多，逐层过滤降成本
...
3. **一次精细筛选 > 两次粗略判断**
   - GLM初筛（免费，快速，准确率80%）
   - DeepSeek/GPT深度分析（付费，详细，准确率95%）
   - 对于Tier 1，直接用95%准确率的方案更合理

4. **用户体验优先**
   - Tier 1内容是用户主动订阅的高质量源
   - 保证不漏好内容 > 节省¥0.01
...
```
Tier 1输入 (10条)
  ↓ GLM-4-Flash初筛（免费，80%准确率）
5条通过
  ↓ DeepSeek/GPT深度分析（付费，95%准确率）
3条输出
```
- 总成本：¥0 + ¥0.01 = ¥0.01
- **问题1**: GLM可能误删5条优质内容（20%误删率）
- **问题2**: 两次判断，增加复杂度和延迟
- **问题3**: 过度优化，为省¥0.01牺牲质量

✅ **正确逻辑**:
```
Tier 1输入 (10条)
  ↓ DeepSeek/GPT深度分析（一步到位，95%准确率）
6条输出
```
- 总成本：¥0.02
- **优势1**: 一次精准判断，准确率更高
- **优势2**: 不漏掉好内容（误删率仅5%）
- **优势3**: 流程简单，延迟更低
- **成本差**: 只多花¥0.01，但用户体验更好

**核心原则**: Tier 1是高质量源，宁可多花小钱，不能漏掉好内容！
